module.exports = function () {
    li('cappy.js'); //receives uploaded image file,assigns it to ob: 'image',creates a db image ob, assigning it the express image-ob  and sets 'path' = that-file's-public-URL//d(image, 'image');
//cappy
//sea all imgs  (template/user gets all his image objects from the db
    a.g('/cappy', MW.lU, function (q, p, n) {
        l('cpy');
        d(U);                                                                                 // AND the path to a 'selected ' image FILE)
        m.image.find({user: q.ud}, function (err, imgs) {  //m.image.find
            p.r('sites/cappy', {imgs: imgs})
        })
    })
    mw = function () {
        m.image.find({user: q.ud}, function (err, imgs) {
            q.imgs = imgs
        })
    }
    //focus on one img
    a.g('/cappy/:imgP', MW.lU, MW.lI, function (q, p, n) {
        l('cappy/:iP');
        d(U);                                                                              //   - template/user gets all his image objects from the db AND the path to a 'selected ' image FILE
        m.image.find({user: q.ud}, function (err, imgs) {
            p.r('sites/cappy/img', {imgs: imgs, imgPath: q.params['imgPath']})
        })
    })
    // upload handler
    a.p('/cappy', MW.lU, function (q, p, n) {
        l('p.cpy');
        d(q.s);
        d(q.files);
        var img = q.files.image;
        var imgObj = {
            user: u, fileObject: img, originalFileName: img.name,
            path: path.basename(img.path)
        };
        d(imgObj);
        m.image.create(imgObj, function (err, doc) {
            if (err) {
                ...
                p.d("/cappy", {msg: 'file stored at path: ' + path})
            }
        })
    })///
    a.g('/cutty', MW.lU, MW.lI, function (q, p, n) {
        li('cty');
        p.r('sites/cutty', {imgs: imgs})
    })
    a.g('/cutty/:imgP', MW.lU, MW.lI
    function (q, p, n) {
        l('cty/:iP');
        p.r('sites/cutty/img', {imgs: imgs, imgP: q.params['imgP']})
    }

    )
 
    // ***
    a.post('/cutty', MW.lU, function (q, p, n) {
        l('cty.p');
        var img = q.files.image;
        var pth = path.basename(img.path)
        m.image.create({
            user: q.ud, fileObject: img,
