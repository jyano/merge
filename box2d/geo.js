...
pD.plus = function (x, y) { return M.p(_.m(this.vs(), function (v) { return V(v).add(x || 0, y || 0) }) ) }
pD.minus = function (x, y) { return M.p( _.m(this.vs(), function (v) {return V(v).sub(x || 0, y || 0) }) ) }
...
w.polU = function (x, y, p1, p2) { return  this.pol(x, y, M.p(p1).U(p2) ) }

w.polD = function (x, y, p1, p2) { return this.pol(x, y,  M.p(p1).D(p2) ) }
...
// with its (potentially) 'slimmer' self
	...
	if (g.n) {g.f.kill() }
	if (g.p) {g.f.dyn()} //can optionally delete the 'something'
...
w.polS = function () {
	...
	if (g.A) { return w.pol.apply(w, g.f) }
	
	if (O(g.f) && O(g.s)) {
		g.e(function (g) { w.pol(g).stat() })
		return w
	}
	
	o = O(g.f) ? g.f : N(g.f) ? {x: g.f, y: g[1], p: g[2]} : {p: g.f}
	o.x = N(o.x, w.hW); o.y = N(o.y, w.hH)
...
f.A = f.area = function () {return M.p(this).getArea()}
b.wV = function () {return this.pD().vs()}
b.rV = function () {
...
b.P = b.pD = function () {var b = this, p, fs = b.fs()
...
b.reg = b.rel = function (p) {
	...
	return _.m(vs, function (v) { v = V(v); return V(v.x - b.X(), v.y - b.Y()) })
...
b2d.hV = b2d.hasVerts = function (poly) { return poly.m_List.get(0) }
...
b.copy = function (x, y) { return this.W().pol(x, y, this) }
...
$dif= DIF = function () {$l('DIF')//not each of the ps?
	var g = G(arguments),
			p = M.p(g.f)
	g.eR(function (p1) {
		p = p.D(p1)
...
w.cxBul = function (k, fn) {var w = this
	
	this.b(function (cx) { cx.w('bul', k, fn) })
}
function later(){
...
pD.ps = function () {var p = this, g = G(arguments), fn,
		ps = [];
	//put all my polys in an array
	_.t(p.nP(), function (i) {  ps.push( p.g(i) )  })
	
	if (b2d.iB(g.f)) {
		ps = _.m(ps, function (p) {
			return p.reg(g.f) })	//return M.p(p).rel(g[0])
		fn = g.s
	}
	
	else if (g.N_) {
		
		ps = _.m(ps, function (p) { return p.reg(g.f, g.s)  })
		fn = g.t
	}
...
pD.ger = function (x, y) { var v = V(x, y); return this.reg(-v.x, -v.y) }
w.ter = function () {
	var w = this, ter = []
	_.t(13, function (i) {
		_.t(8, function (j) {
...
pD.pol = function (b) {alert('pD.pol')
	var p = this
