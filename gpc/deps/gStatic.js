...
stt.clip = function (op, subj, clip, k) {
	...
	/* Build LMT */
	var lmt_table = new LmtTable(), sbte = new ScanBeamTreeEntries(), s_heap = null, c_heap = null;
	if (!subj.isEmpty()) {
	...
	var local_min = lmt_table.top_node, out_poly = new TopPolygonNode(), aet = new AetTree(), scanbeam = 0;
	// out_poly used to create resulting Poly
	/* Process each scanbeam */
	while (scanbeam < sbt.length) {
		/* Set yb and yt to the bottom and top of the scanbeam */
		var yb = sbt[scanbeam++], yt = 0.0, dy = 0.0
		...
		/* Create bundles within AET */
				e0 = aet.top_node, e1 = aet.top_node;
		...
		for (var edge = aet.top_node; (edge != null); edge = edge.next) {
			...
			if ((exists[Clip.CLIP] != 0) || (exists[Clip.SUBJ] != 0)) {
				...
				var contributing = false, br = 0, bl = 0, tr = 0, tl = 0;
...
stt.EQ = function (a, b) {
	return (M.abs(a - b) <= Clip.GPC_EPSILON)
}
stt.PREV_INDEX = function (i, n) {
	return ((i - 1 + n) % n)
}
stt.NEXT_INDEX = function (i, n) {
	return ((i + 1) % n)
}
stt.OPTIMAL = function (p, i) {
	return (p.getY(Clip.PREV_INDEX(i, p.getNumPoints())) != p.getY(i)) ||
			(p.getY(Clip.NEXT_INDEX(i, p.getNumPoints())) != p.getY(i));
}
stt.create_contour_bboxes = function (p) {
	var box = [];
...
stt.minimax_test = function (subj, clip, op) {
...
stt.bound_list = function (lmt_table, y) {
	if (lmt_table.top_node == null) {
...
stt.insert_bound = function (lmt_node, e) {
...
stt.add_edge_to_aet = function (aet, edge) {
...
stt.add_to_sbtree = function (sbte, y) {
...
stt.build_lmt = function (lmt_table, sbte, p, type, op) {
	...
	return edge_table;
} //poly type SUBJ/Clip.CLIP
stt.add_st_edge = function (st, it, edge, dy) {
	if (st == null) {
		...
		if ((edge.xt >= st.xt) || (edge.dx == st.dx) || (M.abs(den) <= Clip.GPC_EPSILON)) {
...
stt.add_intersection = function (it_node, edge0, edge1, x, y) {
	if (it_node == null) {
