...
REGIONS=ANGRYCATS = function(){z()// http://davidsulc.com/blog/2012/04/15/a-simple-backbone-marionette-tutorial/
    cats=[{name: 'Wet Cat'}, {name: 'Bitey Cat'}, {name: 'Surprised Cat'}]
    d = _d()
        .w(500).h(500).c('b').id('content').a()
   ////////////
    ...
    Cat = $$M({}); Cats = $$C({model: Cat}) //Temp(  'angry_cats-template', [thead()( tr().k('header')( th()('Name') ) ),  tbody()]  ).a()

    scrp('angry_cat-template').A(
        $.td(['%= name'])
    ).A()
    ...
    catV = Ma.IV.x({
        t: 'tr',
        tp: "#angry_cat-template",
        k: 'angry_cat'
    })


    catsV = Ma.CmV.x({
        t: "table", id: "angry_cats", className: "table-striped table-bordered",
        tp: "#angry_cats-template",
        itemView: catV,
        appendHtml: function (clVw, itemVw) {
                clVw.$("tbody").A(itemVw.el)
        }
    })

    ap.aI(function (ops) {
        ap.mainRg.show(
            new catsVw({ cl: ops.cats }))
    })
...
MAR00=function(){$.x()
    $.d_(
        ...
        $.art().id('main'), $.scrp('sample-template', 'put some content <%= contentPlacement %>.')
    )
    App = $Ap()
    rg= App.aRg(  "#main" )
    mu= App.mu("SampleModule", function(mu, ap, Bb, ma, $, _){
        var MainVw = ma.IV.x({template: "#sample-template"}), 
            Co  = ma.Co.x({
                initialize: function(opts){this.region = opts.region },
                show: function(){this.region.show(new MainVw({ 
                            model: new Bb.M({contentPlacement: "here"  }) }))}
            })
        mu.addInitializer(function(){ // Initialize this module when the app starts
            mu.co  = new Controller({  region: ap.mainRegion   })
            mu.co.show() })
    })
    App.start()
}
...
// a recursive tree structure for each item in the collection
        TreeRoot = Ma.CollectionView.x({itemView: TreeView})
    }
    CATZ=function(){$.x()
        ...
        Cats=Bb.Collection.x({
            ...
            initialize: function(cats){
                ...
                ap.on('rank:up',function(cat){$l('rank up');
                ...
                ap.on('rank:down', function(cat){$l('rank down');
        ...
        CatView = Ma.ItemView.x({
            ...
            rankUp: function(){
                ap.trigger('rank:up', this.model);
                $l('trigger rank up')},


            rankDown: function(){
                ap.trigger('rank:down', this.model);
                $l('trigger rank down')}


        })
        CatsView = Ma.CompositeView.x({
        ...
        ap.addInitializer(function(options){

            angryCatsView = new CatsView({collection: options.cats})


            ap.mainRegion.show(angryCatsView)
...
// The tree's root: a simple collection view that renders
// a recursive tree structure for each item in the collection
        TreeRoot = Ma.CV.x({itemView: TreeView})
    }
...
function learn(){
    ...
    RNDRR=function(){$.x()
        ...
        /*
         ...
         var data = {foo: "bar"};
         var html = Ma.Renderer.render(template, data);
    ...
    MIV=function(){$.x()
        ...
        my_template_html = '<div><%= args.name %></div>'
        Vw =  Ma.IV.x({ template : function(serialized_model) {n= serialized_model.name;return _.template(my_template_html)({name : n, some_custom_attribute : 'some_custom_key'})}})
        new Vw().render()
        /* Note that using a template function allows passing custom arguments into the .template function and allows for more control over how the .template function is called.
         ...
         While the most common way to render a Bb.Collection is to use
         ...
         always make sense to use these. A Bb.Collection can be
         ...
         */
        MyView =  Ma.IV.x({el: '#my-element', template: false, ui: {paragraph: 'p', button: '.my-button'}, events: {'click @ui.button': 'clickedButton'},
        ...
        /*
         Another use case is when you want to attach a Ma.ItemView to a SVG graphic or canvas element,
         to provide a uniform view layer interface to non-standard DOM nodes. By not having a template
         this allows you to also use a view on pre-rendered DOM nodes, such as complex graphic elements.
         Events and Callback Methods
        ...
        Ma.ItemView.x({onBeforeDestroy: function(){
        ...
        Ma.ItemView.x({ onDestroy: function(){
        ...
        Ma.ItemView.x({serializeData: function(){return {"some attribute": "some value"}}})
        ...
        /*
         ...
         this.ui.elementName:     You can also use the ui hash values from within events and trigger keys using the "@ui.elementName": syntax
         */
        ...
        // modelEvents and collectionEvents // ItemViews can bind directly to model events and collection events     in a declarative manner:
        Ma.IV.x({
    ...
    APRTR=function(){$.x()/*   Ma.AppRouter
     ...
     The route definition is passed on to Bb's standard routing handlers.
     ...
     */
        ...
        /*
         ...
         This allows you to create router instances without having to .x
         ...
         Bb's Router, but has all the same semantics and behavior of the appRoutes
         configuration.
...
//... or (2) in a pam to the constructor
    ...
    FUNCS = function () {
        ...
        /*
         ...
         Bb's extend function is a useful utility to have,
         and is used in
         various places in Ma.

         To make the use of this met more consistent,
         Bb's extend has been aliased to Ma.x.
         This allows
         you to get the extend functionality for your object without having to
         decide if you want to use Bb.View or Bb.Model or another
         Bb object to grab the method from.

         Foo = function(){}

         use Ma.x to make Foo extendable, just like other
         Bb and Ma objects
         Foo.x = Ma.x;
    ...
    BHVR=function(){$.x()
        ...
        DestroyWarn = Ma.Behavior.x({
            // you can set default options
            // just like you can in your Bb Models
        ...
        ToolTip = Ma.Behavior.x({ui: {tooltip: '.tooltip'},
        ...
        //  The default options will be overridden depending on what you set as the options per Behavior (this works just like a Bb.Model).
...
//     The view is a reference to the view instance that the Behavior is on.
    ...
    CMPV=function(){$.x()
        ...
        /*
         ...
         var CompositeView = Ma.CompositeView.x({
        ...
        /*
         ...
         var RowView = Ma.ItemView.x({
         tagName: "tr",
         template: "#row-template"
         });

         var TableView = Ma.CompositeView.x({
         childView: RowView,
