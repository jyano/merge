...
//
    ...
    ACCORD=function(){z()
        $.d().id("accordion").A(
            $.h3('Section 1'),
            $.d().A($.p('afdafssfafsfds Mauris mauris ante, blandit et, ultrices a, suscipit eget.Integer ut neque.' +
            ...
            $.h3('Section 2'),
            $.d().A($.p('Sed non urna. Phasellus eu ligula. Vestibulum sit amet purus.'+
            'Vivamus hendrerit, dolor aliquet laoreet, mauris turpis velit,faucibus interdum tellus libero ac justo')),
            $.h3('Section 3'),
            $.d().A($.p('Nam enim risus, molestie et, porta ac, aliquam ac, risus.'+
                'Quisque lobortis.Phasellus pellentesque purus in massa.'),
                $.ul().A($.li().A('List item 1'), $.li().A('List item 2'), $.li().A('List item 3')))  )
        $("#accordion").accordion()
    }
...
function style() {    UIAN=function(){$.x('x')
...
function state() {    HLFDL=function(){$.x('b', 'hlfdl')
