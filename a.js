...
$methodOverride  = require('method-override'); $cookieParser    = require('cookie-parser');
http = require('http'); path = require('path'); fs = require('fs')
mongoose = require('mongoose')
socketIO = require('socket.io'); sessionSocketIO = require('session.socket.io')
...
$e = express = require('express'); $a = $e(); $a.u = $a.use; $a.g = $a.get; $a.po = $a.post
mongoose.connect("mongodb://localhost/brain", function () {$l('mongo connected')})
$Md = md = $m = models = require('./server/models'); Schema = mongoose.Schema
...
_.e({ port: process.env.PORT || 4000, 'view engine': 'jade', views: __dirname + '/server/views/'},
...
$a.use($session({resave:true, secret: 'foo', saveUninitialized:true,
    store: new MongoStore({ url: 'mongodb://localhost/jy' }) }))
require('./server/routing')(__dirname)
sv = httpServer = http.createServer($a); httpServer.listen(80, function(){$l('server listening on port 80')})
