...
//cvJq()
    function cvJq() {
        ...
        function paths() {
            ...
            cv.bg = cv.begin = cv.beginPath = function (x, y) {
                var cv = this, ctx = cv.ctx()
                ctx.beginPath()
                if (N(x)) {
                    cv.mt(x, y)
            ...
            cv.lt = function (x, y) {
                var cv = this, ctx = cv.ctx(), g = G(arguments)
                if (A(x)) {
                    g.e(
                        function (x) {
                            cv.lt(x[0], x[1])
...
//clear screen [+ fill with color || run fx]
            ...
            cv.horizontalLine = cv.lnh = function (X, Y, l) {
                var g = G(arguments), X = g[0], Y = g[1], l = g[2],
                    X2 = g.p ? X + l : g.n ? X - l : l
                return x.ln(X, Y, X2, Y)
            }
            cv.verticalLine = cv.lnv = function (X, Y, len) {
                var g = G(arguments),
                    X = g[0], Y = g[1], len = g[2],
                    Y2 = g.p ? Y + len
                        : g.n ? Y - len
...
//curves
            ...
            cv.cir = function (X, Y, R, fs, ss) {
                var cv = this, g = G(arguments),
                    X = g[0] || 200, Y = g[1] || 200, r = g[2] || 1, fs = g[3], ss = g[4]
                cv.beginPath()
                cv.arc(X, Y, r, 0, 7, false)
                cv.fillColor('red')
...
//give scale cos(rads), skew:sin(rads)  ??
        ...
        function trans() {
            ...
            cv.tf = function rc(a, c, e, b, d, f) {
                var g = G(arguments)
                if (g.u) {
                    return setTransform(2, 0, 0, 2, 0, 0)
                }
                this.ctx().setTransform(a, c, e, b, d, f)
                return this
            }
        ...
        function drawin() {
            ...
            //draw it where u say, but as if its reg point was its center
            cv.drawRegCenter = cv.dC2 = function f(i, x, y) {
                var that = this
                $.i(i, function (i) {
                    that.draw(i, (that.W() / 2) - (i.W() / 2), (that.H() / 2) - (i.H() / 2))
                })
...
x = ctx = xx = CanvasRenderingContext2D.prototype
xGr = CanvasGradient.prototype
$.c = $.canvas = $.can = function (c, width, h, x, y) {
...
sCan = function (can) {
    ...
    can.sv = can.snap = can.saveImage = function () {
        sv(x)
    }//b
    can.S = can.saveState = function () {
        can.cx.save();
        return can
    }
    ...
    can.dC = function f(i, X, Y) {
        if (U(X)) {//draw img in center of can
            im(i, function (i) {
                can.d(i, (can.w() / 2) - (i.w() / 2), (can.h() / 2) - (i.h() / 2))
            })
        }
        else {//draw it by specifying location of its center
            Y = Y || X
            im(i, function (i) {
                can.d(i, X - i.w() / 2, Y - i.h() / 2)
            })
        }
        return can
    }
...
can2 = function () {
    ...
    can.ftc = function (t, p2) {
        return x.ft(t, x.w() / 2 - x.mt(t) / 2, p2)
    }
    can.Text = function (a, b, c) {
        var g = G(arguments),
            f = function f(a, b) {
                if (S(a) && U(b)) {
                    f.tt = a
    ...
    can.sfl = function (c, f, s, l) {
        x({f: f, s: s, w: l});
        return x
    }
    /// LINES ////
    can.moveTo = x.mt = function (t) {
        return x.x.measureText(t).width
    }
    can.begin = x.bg = function (p1, p2) {
        if (A(p1)) {
    ...
    //draw line ( [x,y],[x,y] || x,y,x,y )
    //draw multiple unconnected lines ([[],[]],[[],[]] || [],[])
    can.ln = function rc(p) {
        var g = G(arguments), p = g[0]
        if (AA(p) || (A(p) && _z(p) == 4)) {
            _e(g, function (p) {
                _a(rc, p)
            })
    ...
    can.lC = x.lc = function (n) {
        n = n || 'r';
        x({p: n});
        return x
    }
    can.lJ = x.lj = function (n) {
        n = n || 'r';
        x({j: n});
        return x
    }
...
// GRADIENTS
    ...
    can.grad2 = function (a, b, c, d, e, f) {
        var g = G(arguments), gr
        if (!N(a)) {
            return x('p', a, pt[b] || null)
        } //pattern
        if (f) {
            g = x('rg', g)
        } else if (d) {
        ...
        if (!N(a)) {
            return x.x.createPattern(a, oO('pt', b) || null)
        }
        if (f) {
            g = x.x.createRadialGradient(a, b, c, d, e, f)
        } else if (d) {
            g = x.x.createLinearGradient(a, b, c, d)
        }
    ...
    can.IntervalGrad = x.gra = function () {
        var a = 0, b = 1000
        I(.02, function () {
            a += 1;
            b -= 2;
    ...
    //TRANSFORM
    //scale MULTIPLIER
    can.scale = x.sc = function (X, Y) {
        var g = G(arguments),
            X = g[0] || 1, Y = g[1] || X
        x.x.scale(X, Y)
    }
    can.translate = x.tr = function (X, Y) {
        var g = G(arguments),
            X = g[0] || 1, Y = g[1] || X
        x.x.translate(X, Y)
    }
    can.rotate = x.rt = function (r) {
        var g = G(arguments),
            r = g[0] || 1
        //r=pi(-6)*r
        //x.fr(0,0,1,1)
    ...
    can.transform = x.tf = function rc(a, c, e, b, d, f) {
        var g = G(arguments)
        if (g.p) {
            return rc(2, 0, 0, 2, 0, 0)
        }
    ...
    can.setTransform = x.stf = function rc(a, c, e, b, d, f) {
        var g = G(arguments)
        if (g.p) {
            return rc(2, 0, 0, 2, 0, 0)
        }
