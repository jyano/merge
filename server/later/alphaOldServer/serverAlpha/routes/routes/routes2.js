$l('routes.js'); module.exports=function(){// $a.get('/wap', function(req,res){ res.render('wap') })
    ...
    $a.post('/login',  function(q,p,n){//$l(q.body.username); $l(q.body.password)
        models.user.findOne({username : q.body.username, password : q.body.password}, function(z,u){
            if(z){n(z)} if(!u){$l('FAIL!!'); p.json('guest')}
            else {//$l('logged in: ' + u + ' !')
                q.session.username = u.username
                q.session.loggedIn = true
                _.extend(q.session, {username: u.username, loggedIn: true}).save(function () {$l('session saved ' + q.session.username + ', ' + u.username)
                    p.json(u.username)})}})
    ...
    $a.get('/user/:username', function(q, p){
    ...
    $a.get('/mugByUsername/:username', function(q,p){   $l('req.params.username: ' + req.params.username)
