...
//    newPath: public/pics/55e32.JPG
    ...
    $a.get('/dcs',function(q,p){$l(" $a.get('/dcs',function(q,p){var res=[]; _.e($dcs, function(dc){ res.push(dc) }); p.json(res)}) ")
        var res=[]; _.e($dcs, function(dc){ res.push(dc) }); p.json(res)})


    $a.post('/dcs', function(q,p){
        $l("$a.post('/dcs', function(q,p){var dc=q.body, did; dc.id= d++; $dcs[ dc.id ] = dc; p.json(dc) })")

        var dc=q.body, did; dc.id= d++; $dcs[ dc.id ] = dc; p.json(dc)
    ...
    $a.put('/dcs/:id', function(q,p){$l("$a.put('dcs/:id', function(q,p){var dc=q.body; $dcs[ q.params.id ] = dc; p.json(dc) })")
        var dc=q.body; $dcs[ q.params.id ] = dc; p.json(dc) })
    ...
    $a.get('/dcs/:did/nts',function(q,p){
       // $l("$a.get('/dcs/:did/nts',function(q,p){ var res=[]; _.e($nts[q.params.id], function(nt){ $l('nt!'); res.push(nt)}); return res })")
        console.log($nts)
        var res=[]; _.e($nts[q.params.did], function(nt){
    ...
    $a.post('/dcs/:did/nts', function(q,p){
     //   $l("var did = q.params.id, nt = q.body; if (!$nts[did]) { $nts[did] = {} }; nt.id = n; $nts[did][n] = nt; n++; p.json(nt) })")
        var did = q.params.id, nt = q.body; if (!$nts[did]) { $nts[did] = {} }; nt.id = n; $nts[did][n] = nt; n++; p.json(nt) })

    $a.put('/dcs/:did/nts:nid', function(q,p){$l("$a.put('dcs/:did/nts:nid', function(q,p){ var nt=q.body; $nts[q.params.did][q.params.nid] = nt; p.json(nt) })")
        var nt=q.body; $nts[q.params.did][q.params.nid] = nt; p.json(nt) })
