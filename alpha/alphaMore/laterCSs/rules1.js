...
$rule=  $ruleStr=  function (sel, decs, rulesOb){
    ...
    $decBlk= $decBlkObToStr=  function (decs, rulesOb) {


        $decs= function (decs, rulesOb) {
            var DECS = {}; advanced()
            _.e(decs,function(v,k){if (k !== 'mx' && k !== 'ex'){if(A(v)){DECS[oO('s', k)] =   $CSS.fn[k].apply(null, v); return}//plugin

                DECS[oO('s', k)] = N(v) ? String(v) + 'px' :
                    oO(k, v, 'R')
            ...
            function advanced() {
                ...
                if (decs.ex) {
                    alert('$CSS extend')
                    if (rulesOb && rulesOb[decs.ex]) {
                        _.x(decs, rulesOb[decs.ex])
                    }
                    else {
                        _.x(decs, $CSS.ex[decs.ex])
                    }//tries to extend from one of its own first //then goes to global ($CSS.ex)
                }
                //mixin
                if (decs.mx) {
                    alert('$CSS mixin')
        ...
        } // pops a decsOb using cssObject= $decBlkObParse



        var decBlkStr,decs;
        if (S(decs)) {$l('($CSS.)$decBlk S(decs');
            return decs
        } //for @media ??
        decBlkStr=' '
        decs= $decs(decs, rulesOb) // only for ex,mx,fn
        _.e(decs, function (v,k){
            decBlkStr += k+':'+v+'; '
        })
        return   '{'  +decBlkStr+ '}'

    } //turns decObj to string
    ...
    if (A(sel)) {alert('$rule array')
        ...
        str=''
        _.e($decBlkObParse(sel), function (v, k) {str +=  k+   ':' + v  +  '; '})
        sel = '{'+ str +'}'}
...
$CSS =  function(rulesOb){var  rulesStr
...
$CSS.bor= function(c){alert('$CSS.bor'); var  col = oO('c', c|| 'z');return '8px solid ' + col}
