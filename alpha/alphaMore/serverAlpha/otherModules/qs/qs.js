...
var internals={depth: 5, arrayLimit: 20, parametersLimit: 1000}
internals.parseValues = function(str) {
    ...
    for (var i=0,il=parts.length;  i<il;  ++i) {
        ...
        if (pos === -1){obj[Utils.decode(part)] = ''}
...
internals.parseObject=function(chain, val){
    ...
    if(root==='[]'){
        ...
        else {obj[cleanRoot] = internals.parseObject(chain, val)}
    }
...
internals.parseKeys = function(key, val, depth) {
    ...
    depth = U(depth)? internals.depth : depth
    ...
    if (Object.prototype.hasOwnProperty( segment[1] )){return}

    // Stash the parent if it exists

    var keys=[]

    if(segment[1]){keys.push(segment[1])}
